...
const formatJobDate = date => moment(date).format('YYYY');
                ...
                {resume.map((role, roleIndex) => (
                    <li key={roleIndex} className="flex gap-4">
                        <div
                            ...
                            <Image src={process.env.NODE_ENV === 'development' ? process.env.SERVER_URL + role.logo : role.logo} alt="" className="h-7 w-7 rounded-full" width={32} height={32} unoptimized/>
                        </div>
...
export default function Home({ page }) {
        ...
        <>
            <Head>
                <title>
                    Wayne Foster - Cloud Developer
                </title>
                <meta
                    name="description"
                    content="I’m Wayne, a software developer and entrepreneur based in Atlanta, GA."
                />
            </Head>
            ...
            <Container className="mt-24 md:mt-28">
                <div className="mx-auto grid max-w-xl grid-cols-1 gap-y-20 lg:max-w-none lg:grid-cols-2">
                    <div className="flex flex-col gap-16">
                        {page?.attributes?.posts?.data?.map((post) => (
                            <Article key={post.id} article={post}/>
                        ))}
                    </div>
                    <div className="space-y-10 lg:pl-16 xl:pl-24">
                        <Newsletter/>
                        <Resume jobs={page?.attributes?.resume?.data} />
                    </div>
                </div>
            </Container>
        </>
